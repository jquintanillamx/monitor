  ...
  render() {
    this.innerHTML = `
      <div class="monitoreo-widget">
        <p>Se monitoreará toda la página</p>
  ...
  async handlePomDetailsWidget(widget) {
    setTimeout(async () => {
      ...
      if (moreInfoButton) {
        ...
        this.logChange('Botón "More Information" clicado.');
        ...
        setTimeout(async () => {
          ...
          this.logChange(`Número de Tarjeta: ${this.numeroTarjeta}`);
         ...
         /*  window.cuenta = this.cuenta;
          ...
          if (seeLessButton) {
            seeLessButton.click();
            this.logChange('Botón "See Less Information" clicado.');
        ...
        this.logChange('Botón "More Information" no encontrado.');
      }
    }, 500);
  }
    ...
    if (this.cancelButton) {
      this.cancelButton.addEventListener("click", async () => {
        if (document.body.contains(this.cancelButton)) {
          this.logChange('Botón "Cancel Preview" clicado.');
    ...
    if (this.callButton) {
      this.callButton.addEventListener("click", () => {
        if (document.body.contains(this.callButton)) {
          this.logChange('Botón "Dial" clicado.');
    ...
    if (summaryElement) {
      this.customStateObserver = new MutationObserver(() => {
        ...
        if (this.panelStatus === "Dialing") {
          // Restablecer cancelPreviewActive si está en true
          if (window.cancelPreviewActive) {
            window.cancelPreviewActive = false;
          }

          // Si el monitoreo del custom panel está detenido, volver a iniciarlo
          if (!this.customPanelObserver) {
            this.monitorCustomPanel();
            this.logChange("Monitoreo del custom panel reiniciado.");
          }

          // Ejecutar la lógica de handleDialClick
          this.handleDialClick(
            this.cuenta,
            this.firstName,
            this.numeroTarjeta,
          ...
          // Esperar el fin de la llamada para cargar el botón
          this.waitForEndOfCall(panel).then(() => {
            this.logChange("Fin de llamada detectado. Se procederá a modificar los botones.");
...
/*   monitorPanelStatus(panel) {
  ...
  async stopCustomPanelObserver() {
    return new Promise((resolve) => {
      if (this.customPanelObserver) {
        this.customPanelObserver.disconnect();
        this.customPanelObserver = null;
        this.logChange("Dejó de monitorear el custom panel.");
  ...
  monitorWorkArea() {
    ...
    if (workArea) {
          ...
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach((node) => {
              if (node.nodeName === "CARD") {
                ...
                this.logChange(
                  `Nueva interacción detectada con ID: ${node.getAttribute(
                    "interaction-id"
                  )}`
                );
      ...
      this.logChange("No se encontró el ws-work-area en el DOM.");
    }
  }

  async handleInteraction(interaction) {
    this.logChange(
      `Interacción detectada con ID: ${interaction.getAttribute(
        "interaction-id"
      )}`
    );
    window.callEnded = false;
...
customElements.define("monitor-widget", Monitor);
